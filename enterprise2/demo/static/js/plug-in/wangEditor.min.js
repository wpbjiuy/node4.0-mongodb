...
$.extend($E, {
    'getUploadImgComponentForCrossDomain': function(editor){
        ...
        $uploadImg_modal.find('#' + btnId).click(function(e){
            ...
            /*自定义上传图片开始——王平波*/ 
            if (fileVal.length) {
                var extension = fileVal.substring(fileVal.lastIndexOf('.'), fileVal.length).toLowerCase();
                if (extension == ".jpg" || extension == ".png") {
                    var fileImgData = new FormData();
                    fileImgData.append('upload', $('#' + fileId)[0].files[0]);
                    zfnFsupload(fileImgData)
                } 
            }

            function zfnFsupload(data){
                $.ajax({
                    url: uploadUrl,
                    type: 'POST',
                    data: data,
...
'insertImage': {
    ...
    'modal': function (editor) {
        ...
        var content =   '<p>' +
                        '   <span class="active" id="' + alertInfoId + '">' +langInsertWeb+ '</span> ' +
                        '   <a href="#" id="' + changeLinkId + '"></a>' +
                        '</p>' +
                        '<div id="' + webImgContainerId + '">' +
                        '   <p>' +langUrl+ '：<input id="' + urlTxtId + '" type="text" style="width:300px;" placeholder="http://"/>'+
                        '     <span class="wangEditor-modal-btn" onclick="WdshowFixedInfo2($(this),\'imagesdir\',\'/static/CPM/slcImgOne.html\')">选择已有图片</span></p>' +
                        '   <p>' +langTitle+ '：<input id="' + titleTxtId + '" type="text" style="width:300px;"/></p>' +
                        '   <p class="imgBox"></p>' +
                        '<p><button id="' + btnId + '" type="button" class="wangEditor-modal-btn">' +langBtn+ '</button></p>' +
                        '</div>',
