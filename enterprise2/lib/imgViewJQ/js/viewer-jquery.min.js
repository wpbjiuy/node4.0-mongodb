...
function(i) {
    ...
    function l(i, t) {
        var e,src;
        if(jQuery(i).hasClass('canvasview')){
            src = jQuery(i).attr('data-src')
            viewCanvas(jQuery(i),src,t)
        }
        return i.naturalWidth ? t(i.naturalWidth, i.naturalHeight) : (e = document.createElement("img"), e.src = src, e.onload = function() {
    ...
    ei = Math.max,
    si = Number,
    isIe = navigator.appName == "Microsoft Internet Explorer" ||　navigator.userAgent.indexOf(".NET4.0E; rv:11.0") != -1 && !(navigator.userAgent.indexOf("Opera") > -1);
    u.prototype = {
        constructor: u,
        init: function() {
            var t = this.options,
        ...
        initImage: function(t) {
            ...
            a = n.width,
            h = ei(n.height - o, o),
            g = this.$images[this.index].imageStyle || {},
            r = !this.isViewed && g ? g : (this.image || {});
            l(s[0], i.proxy(function(s, n, iscv) {
                var o, l, d = s / n,
                c = a,
                u = h;
                h * d > a ? u = a / d: c = h * d,
                ...
                u = !iscv ? ti(.9 * u, n) : n,
                l = {
                    naturalWidth: r.naturalWidth || s,
                    naturalHeight: r.naturalHeight || n,
                    aspectRatio: r.aspectRatio || d,
                    ratio: r.ratio || (c / s),
                    width: r.width || c,
                    height: r.height || u,
                    left: r.left || ((a - c) / 2),
        ...
        renderImage: function(t) {
            ...
            this.$images[this.index].imageStyle = e;
        ...
        hide: function() {
        	var thsImageStyle = i.extend({},true,{},this.$images[this.index].imageStyle);
            var t = this.options,
            e = this.$viewer;
            ...
            this.$images[this.index].imageStyle = thsImageStyle;
        },
        view: function(t) {
            var e, s, n, o, a, cv, h = this.$title;
            t = Number(t) || 0,
            !this.isShown || this.isPlayed || 0 > t || t >= this.length || this.isViewed && t === this.index || this.trigger(Z).isDefaultPrevented() || (s = this.$items.eq(t), n = s.find(E), o = n.data("originalUrl"), a = n.attr("alt"),
            cv = s.find('img').attr('data-original-url').lastIndexOf('tif') == -1 || isIe, 
            this.$image = e = cv ? i('<img src="' + o + '" alt="' + a + '">') : i('<div class="canvasview" data-src="' + o + '"><p class="loading">loading...</p></div>'), 
            this.isViewed && this.$items.eq(this.index).removeClass(k), 
            s.addClass(k), this.isViewed = !1, this.index = t, this.image = null, this.$canvas.html(e.addClass(I)), 
            this.renderList(), h.empty(), 
            ...
            this)), (e[0].complete || !cv) ? this.load() : (e.one(M, i.proxy(this.load, this)), this.timeout && clearTimeout(this.timeout), this.timeout = setTimeout(i.proxy(function() {
                e.removeClass(I),
                this.timeout = !1
            },
            this), 1e3)))
        ...
        zoom: function(i, t, e) {
            ...
            i = si(i),
            i = 0 > i ? 1 / (1 - i) : 1 + i;
            var iscv = this.$image.hasClass('canvasview');
            var cvi = i < 1 ? -0.05 : 0.05
            if(iscv)
                this.zoomTo(s.scaleX+cvi, t, e)
            else
        ...
        zoomTo: function(i, t, s, n) {
            ...
            g = v.height,
            sl = i,
            iscv = this.$image.hasClass('canvasview');
            if(iscv){
                v.scaleX = v.scaleY = i
            }
            i = ei(0, i),
            ...
            !iscv?(v.left -= (a - f) * ((l.pageX - r.left - v.left) / f)):'', 
            !iscv?(v.top -= (h - g) * ((l.pageY - r.top - v.top) / g)):'') : (v.left -= (a - f) / 2, v.top -= (h - g) / 2), !iscv ? (v.width = a) : '', !iscv ? (v.height = h) : '', v.ratio = i, this.renderImage(), t && this.tooltip())
        },
        rotate: function(i) {
