...
$.extend($E, {
    ...
    'addImgMaxWidth': function ($content,$style) {
        $content.find('img').each(function(){
            ...
            if(!markValue){
                ...
                if($style){
                    if($style.width){
                        $this.css('width', $style.width);
                    }
                    if($style.height){
                        $this.css('height', $style.height);
                    }
                }
...
$.extend($E, {
    ...
    'htmlTemplates': {
        //删除table,img的按钮
        'elemDeleteBtn': '<a href="javascript:void(0)" class="wangEditor-elemDeleteBtn"><i class="icon-wangEditor-cancel"></i></a>',
        ...
        'btn': '<a class="wangEditor-btn-container-btn wangEditor-btn-container-btn-default" href="javascript:void(0)"></a>', 
        //下拉按钮右侧的小三角
        ...
        'modalHeader': '<div class="wangEditor-modal-header clearfix">' + 
                            '<a href="javascript:void(0)" commandName="close" class="wangEditor-modal-header-close"><i class="icon-wangEditor-cancel"></i></a>' + 
                            '<b>{title}</b>' + 
        ...
        //编辑框
        'txt': '<div class="wangEditor-textarea" contenteditable="true" string-html="contentData.details"><p><br/></p></div>',
        //dropmenu
...
$.extend($E, {
    'getUploadImgComponentForCrossDomain': function(editor){
        var uploadUrl = editor.uploadUrl,
            fileInputName = 'upload',  //服务器端根据这个name获取file
        ...
        var content =   '<form id="' + formId + '" method="post" enctype="multipart/form-data" target="' + iframeId + '">'+
                        '   <p>' +langChoose+ '：<span class="file_btn">请选择图片文件<input type="file" accept="image/*" name="' + fileInputName + '" id="' + fileId + '" /></span></p>' +
                        '   <p>' +langTitle+ '：<input type="text" id="' + titleTxtId + '" style="width:250px;"/></p>' +
                        '   <p class="imgBox"></p>' +
                        '   <p>宽度：<input type="number" class="imgw" style="width:100px;margin-right:4px;"/> 高度：<input type="number" class="imgh" style="width:100px;"/></p>' +
        ...
        $uploadImg_modal.find('#' + fileId).change(function(){
            var fileValue = $(this).val();
            var fileSize = 0;
            var imgBox = $(this).parents('form').find('.imgBox')
            var imgSrc = "";
        ...
        $uploadImg_modal.find('#' + btnId).click(function(e){
            ...
            var imgw =  $uploadImg_modal.find('#imgw').width()
            var imgh =  $uploadImg_modal.find('#imgh').height()
            var thsStyle = 'width:'+imgw+'px;height:'+imgh+'px;';
            //定义callback事件
                ...
                if(result.indexOf('ok|') === 0){
                    ...
                    if(arr.length === 2){
                        ...
                        if(title === ''){
                            ...
                            editor.command(e, 'customeInsertImage', {'url':url, 'title':title, 'style':thsStyle}, uploadImg_callback);
                        }
                        ...
                        editor.command(e, 'customeInsertImage', {'url':thumbnailUrl, 'link': url, 'title':title, 'style':thsStyle}, uploadImg_callback);
                    }else{
            ...
            /*自定义上传图片开始——王平波*/ 
            cm.uploadImg($('#' + fileId), uploadUrl, function(data){
                window.wangEditor_uploadImgCallback('ok|'+data.msg)
                $btn.show();
                $info.html('');
                $btn.parents('form').find('.imgBox').children().remove()
            },function(data){
                $btn.show();
                $info.html('');
                $btn.parents('form').find('.imgBox').children().remove()
...
'justify': {
    ...
    'dropMenu': function(){
        var arr = [],
            temp = '<li><a href="javascript:void(0)" customCommandName="${command}">${txt}</a></li>',
            $ul,
...
'fontFamily': {
    ...
    'dropMenu': function(){
        var arr = [],
            ...
            temp = '<li><a href="javascript:void(0)" commandValue="${value}" style="font-family:${family};">${txt}</a></li>',
            $ul;
...
'fontSize': {
    ...
    'dropMenu': function () {
        var arr = [],
            //注意，此处commandValue必填项，否则程序不会跟踪
            temp = '<li><a href="javascript:void(0)" commandValue="${value}" style="font-size:${fontsize};">${txt}</a></li>',
            $ul;
...
'setHead': {
    ...
    'dropMenu': function(){
        ...
        var liListStr =  '<li><a href="javascript:void(0)" commandValue="<h1>"><h1>' + head + '1</h1></a></li>' + 
                    '<li><a href="javascript:void(0)" commandValue="<h2>"><h2>' + head + '2</h2></a></li>' + 
                    '<li><a href="javascript:void(0)" commandValue="<h3>"><h3>' + head + '3</h3></a></li>' + 
                    '<li><a href="javascript:void(0)" commandValue="<h4>"><h4>' + head + '4</h4></a></li>' + 
                    '<li><a href="javascript:void(0)" commandValue="<p>">' + content + '</a></li>';
...
'foreColor': {
    ...
    'dropPanel': function(){
        var arr = [],
            //注意，此处commandValue必填项，否则程序不会跟踪
            temp = '<a href="javascript:void(0)" commandValue="${value}" style="background-color:${color};" title="${txt}" class="forColorItem">&nbsp;</a>',
            $panel;
...
'insertExpression': {
    ...
    'dropPanel': function(editor){
        ...
        var $panel,
            temp = 
                //注意，此处commandValue必填项，否则程序不会跟踪
                '<a href="javascript:void(0)" commandValue="${value}">' + 
...
'insertImage': {
    ...
    'modal': function (editor) {
        ...
        var content =   '<p id="show'+webImgContainerId+'">' +
                        '   <span class="btn-1 active" id="' + alertInfoId + '">' +langInsertWeb+ '</span> ' +
                        '   <span class="btn-1" id="' + changeLinkId + '">上传图片</span>' +
                        '</p>' +
                        '<div id="' + webImgContainerId + '">' +
                        '   <p>' +langUrl+ '：<input id="' + urlTxtId + '" type="text" style="width:300px;" placeholder="请输入网络图片地址，或选择已有图片地址"/>'+
                        '     <span id="slcImg' + urlTxtId + '" class="wangEditor-modal-btn">选择已有图片</span></p>' +
                        '   <p>' +langTitle+ '：<input id="' + titleTxtId + '" type="text" style="width:300px;"/></p>' +
                        '   <p class="imgBox"></p>' +
                        '<p><button id="' + btnId + '" type="button" class="wangEditor-modal-btn">' +langBtn+ '</button></p>' +
                        '</div>'
        ...
        $(function(){
            var $webImgContainerId = $webimg_modal.find('#' + webImgContainerId),
                $alertInfoId = $webimg_modal.find('#' + alertInfoId),
                $changeLinkId = $webimg_modal.find('#' + changeLinkId),
                $showFom = $webimg_modal.find('#show' + webImgContainerId),
                $slcImg = $webimg_modal.find('#slcImg' + urlTxtId),
                $urlTxt = $webimg_modal.find('#' + urlTxtId);
            ...
            //切换
            $showFom.find('span.btn-1').click(function(e){
                var txt = $(this).text();
                $(this).addClass('active').siblings().removeClass('active')
                if(txt !== langInsertWeb){
                    showUploadImg();
                }else{
                    showWebImg();
                }
            ...
            // jiuy add
            $slcImg.click(function(e){

                slcImgObj.slcImgs(e,$urlTxt)
...
'insertCode':{
    ...
    'modal': function(editor){
        ...
        var content = '<p>' +langTitle+ '：{selectLangs}</p>' +  // selectLangs 待填充语言列表下拉框
                        '<div><textarea id="' + txtId + '" style="width:100%; height:100px;" ng-model="contentData.describe"></textarea></div>' + 
    ...
    */
    'wangEditor': function(options){
        ...
        .click(function(e){
            ...
            //转译引号
            sourceCode = $E.replaceQuotes(sourceCode);
            $('#' + $E.showCodeTextareaId).val( sourceCode );
        });
