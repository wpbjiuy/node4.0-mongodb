...
cm.fnDirList = function (obj,callback){
	...
	_self.update = function(type,thsLi,data,name){
		switch(type){
			case 'delete':
	...
	_self.updateList = function(thsLi,data,name){
		var _setA = thsLi.find('>p>a').text(name),
				_path = _setA.attr('data-path');
		var _d;
		if(_path.indexOf('-') > 0){
			var _ar = _path.split('-');
			_d = this.data[_ar[0]];
			for (var i = 1; i < _ar.length; i++) {
				_d = _d[this.nextData][_ar[i]];
			}
		}else{
			_d = this.data[_path];
		}
	...
	_self.getOnData = function (path) {
		return cm.getPathData(path, this.data, this.nextData && this.nextData.dataPath, true).data;
	}
	...
	_self.addLists = function(addDom, data, isFirstAdd, pd, dirName){
		...
		if(isFirstAdd && ths.on !== undefined){
			var _onDom = ths.ul;
			if($.isArray(ths.on)){
